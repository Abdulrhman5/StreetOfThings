...
{
    public interface IObjectCommentDeleter
    {
        Task<CommandResult> AuthorizedDeleteComment(DeleteCommentDto deleteCommentDto);
    }
    ...
    class ObjectCommentDeleter : IObjectCommentDeleter
    {
        ...
        public async Task<CommandResult> AuthorizedDeleteComment(DeleteCommentDto deleteCommentDto)
        {
            ...
            if (comment == null)
            {
                return new CommandResult(new ErrorMessage
                {
                    ErrorCode = "CATALOG.OBJECT.COMMENT.DELETE.INVALIDID",
